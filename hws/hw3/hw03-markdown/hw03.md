...
\end{aligned}
$$
in which $\mathbb{1}_{x_i\geq 0}$ is the indicator function, $\boldsymbol{x}=(x_1,\ldots,x_m)^{T}$, $\boldsymbol{y}=(y_1,\ldots,y_m)^{T}$. The expected complete-data log-likelihood at timestep $t$ is
$$
\begin{aligned}
