...
$$
\frac{\partial}{\partial(\sigma^2)}\mathcal{l}(\theta^{(t)})=\frac{\partial}{\partial(\sigma^2)}Q^{(t)}(\theta^{(t)})=\sum_{i=1}^{m}\Big(\frac{y_i^2+(\mu^{(t)})^2-2(2\gamma^{(t)}_{i,1}-1)y_i\mu^{(t)}}{2((\sigma^2)^{(t)})^2}-\frac{1}{(\sigma^2)^{(t)}}\Big)\tag{30}
$$
